...
import { calculateStats, calculatePartyEffectiveProfit } from '@/lib/statsService'
...
export default function Dashboard() {
  ...
  const [totalWithdrawals, setTotalWithdrawals] = useState<Withdrawal[]>([])
  ...
  const loadPartners = async () => {
    ...
    try {
      const [partnerData, withdrawalData] = await Promise.all([
        partnerService.getPartners(activeWorkspaceId),
        partnerService.getAllWithdrawals()
      ])
      setPartners(partnerData)
      setTotalWithdrawals(withdrawalData)
    } catch (error) {
      console.error('Error loading partners/withdrawals:', error)
    }
  }
  ...
  const loadOrders = async (latestInvestment?: InvestmentRecord | null) => {
    ...
    try {
      ...
      // This ensures consistency with the "Party" view which uses:
      // - Full Estimated Profit if outstanding > 250
      // - "Remaining Profit" (Realized) if outstanding <= 250

      const profitReceived = calculatePartyEffectiveProfit(filteredOrders, ledgerEntriesInRange)
  ...
  return (
    <AuthGate>
      <div className="bg-gray-50" style={{
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-gray-500">Based on Total Order Profit</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-[10px] text-gray-400 uppercase tracking-wider">Total Profit</p>
                    <p className="text-lg font-bold text-green-600">{formatIndianCurrency(stats.profitReceived || 0)}</p>
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  {partners.length > 0 ? (
                    partners.map((partner) => {
                      const partnerShare = (stats.profitReceived || 0) * (partner.percentage / 100)

                      // Calculate withdrawals for this partner
                      const partnerWithdrawals = totalWithdrawals.filter(w => w.partnerId === partner.id)
                      const withdrawnAmount = partnerWithdrawals.reduce((sum, w) => sum + w.amount, 0)
                      const netPayable = partnerShare - withdrawnAmount
                      ...
                      return (
                        <button
                          ...
                          className="w-full group block bg-gray-50 hover:bg-gray-100 rounded-xl p-3 transition-all active:scale-[0.98] border border-transparent hover:border-gray-200"
                        >
                          <div className="flex justify-between items-center mb-2">
                            <div className="flex items-center gap-2">
                              ...
                              <div className="text-left">
                                <p className="text-sm font-semibold text-gray-900 group-hover:text-primary-700 transition-colors">{partner.name}</p>
                                <p className="text-[11px] text-gray-500">{partner.percentage}% Share</p>
                              </div>
                            </div>
                            <div className="text-right">
                              <p className="text-sm font-bold text-gray-900">{formatIndianCurrency(partnerShare)}</p>
                            </div>
                          </div>

                          {/* New UI: Withdrawn & Available - No Progress Bar */}
                          <div className="flex justify-between items-center text-xs pt-2 border-t border-gray-100 mt-2">
                            <div className="text-red-500 font-medium">
                              Withdrawn: {formatIndianCurrency(withdrawnAmount)}
                            </div>
                            <div className="text-green-600 font-bold bg-green-50 px-2 py-0.5 rounded-lg border border-green-100">
                              Available: {formatIndianCurrency(netPayable)}
                            </div>
                          </div>
                        </button>
                      )
                    ...
                    <div className="text-center py-6 text-gray-400 text-sm bg-gray-50 rounded-xl border border-dashed border-gray-200">
                      ...
                      <button
                        ...
                        className="text-primary-600 font-medium text-xs mt-1 hover:underline"
                      >
                        Go to Account to add partners
                      </button>
                    </div>
                  )}
                </div>
        ...
        {selectedPartner && (
          <PartnerDetailDrawer
            isOpen={showPartnerDrawer}
            onClose={() => setShowPartnerDrawer(false)}
            partner={selectedPartner}
            totalProfit={stats.profitReceived || 0}
            onWithdrawalsChange={loadPartners}
          />
        )}
      </div >
    </AuthGate>

...
export const ledgerActivityService = {
  ...
  async logActivity(activity: Omit<LedgerActivity, 'id' | 'timestamp'>): Promise<string> {
    ...
    if (activity.partnerId !== undefined) cleanActivity.partnerId = activity.partnerId || null
    if (activity.previousPartnerId !== undefined) cleanActivity.previousPartnerId = activity.previousPartnerId || null
  ...
  async getActivities(startDate?: string, endDate?: string): Promise<LedgerActivity[]> {
    ...
    snap.forEach((doc) => {
      ...
      activities.push({
        ...
        previousPartnerId: data.previousPartnerId,
        type: data.type,
